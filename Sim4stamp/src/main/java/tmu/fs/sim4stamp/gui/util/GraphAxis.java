...
public class GraphAxis {
    ...
    public double[] getScale(double max, double min, GraphData.GhType ghType) {
        ...
        if (max > 0) {
            if (min >= 0.0) {
                pow = getPow(max);
                axizIndex = getAxisIndex(pow, max, ghType);
                retList = axisList[axizIndex];
            } else {
                if ((-1) * min > max) { // 最低値がマイナスで絶対値がmaxより大きい
                    pow = getPow((-1) * min);
                    axizIndex = getAxisIndex(pow, (-1) * min, ghType);
    ...
    private static int getAxisIndex(int pow, double value, GraphData.GhType ghType) {
        double norm = value / Math.pow(10, pow);
        if (ghType != null && ghType == GraphData.GhType.INT && value < 10.0) {
            for (int i = 5; i >= 0; i--) {
